...
def main():
    ...
    try:
        ...
        if args.mode == 'live':
            ...
            results = engine.run_backtest(args.start_date, args.end_date, use_vectorbt=args.vectorbt)
            ...
            logger.info("Starting A/B test mode", start_date=args.start_date, end_date=args.end_date)
            ...
            if 'error' in results:
                ...
                if comparison:
                    ...
                    print(f"  Total Return: {enabled_capital.get('total_return', 0):.2%}")
                    print(f"  Sharpe Ratio: {enabled_capital.get('sharpe_ratio', 0):.3f}")
                    print(f"  Max Drawdown: {enabled_capital.get('max_drawdown', 0):.2%}")
                    ...
                    print(f"  Total Return: {disabled_capital.get('total_return', 0):.2%}")
                    print(f"  Sharpe Ratio: {disabled_capital.get('sharpe_ratio', 0):.3f}")
                    print(f"  Max Drawdown: {disabled_capital.get('max_drawdown', 0):.2%}")
                    print(f"  Win Rate: {disabled_capital.get('win_rate', 0):.2%}")

                    print("\\n--- IMPROVEMENT (Enabled - Disabled) ---")
                    print(f"  Sharpe Ratio: {comparison.get('sharpe_ratio_improvement', 0):.3f}")
                    print(f"  Total Return: {comparison.get('total_return_improvement', 0):.2%}")
                    print(f"  Max Drawdown: {comparison.get('max_drawdown_improvement', 0):.2%}")
                    print(f"  Win Rate: {comparison.get('win_rate_improvement', 0):.2%}")
                    print(f"  Conviction Correlation: {comparison.get('conviction_correlation', 0):.3f}")

                    logger.info("A/B test completed successfully",
                              sharpe_improvement=comparison.get('sharpe_ratio_improvement', 0),
                              return_improvement=comparison.get('total_return_improvement', 0))
                else:
                    logger.warning("A/B test completed but no comparison data available")

        elif args.mode == 'walk-forward':
            logger.info("Starting walk-forward validation mode", start_date=args.start_date, end_date=args.end_date)
            ...
            if 'error' in results:
                ...
                print(f"Training Window: {results.get('train_window_months', 0)} months")
                ...
                if folds:
                    ...
                    for fold in folds[:5]:  # Show first 5 folds
                        ...
                        print(f"Fold {fold.get('fold', 0)}: Sharpe Gap {perf_gap.get('sharpe_gap', 0):.3f}, "
            ...
            logger.info("Starting survivor-free backtest mode", start_date=args.start_date, end_date=args.end_date)
            ...
            if 'error' in results:
                ...
                print(f"Period: {args.start_date or 'Default'} to {args.end_date or 'Default'}")

                if survivor_analysis:
                    print("\\n--- Survivor Analysis ---")
                    print(f"Original Universe: {survivor_analysis.get('original_universe_size', 0)} assets")
                    print(f"Survivor Universe: {survivor_analysis.get('survivor_universe_size', 0)} assets")
                    print(f"Survival Rate: {survivor_analysis.get('survival_rate', 0):.2%}")
                    excluded = survivor_analysis.get('excluded_assets', [])
                    if excluded:
                        print(f"Excluded Assets: {', '.join(excluded[:5])}{'...' if len(excluded) > 5 else ''}")
                ...
                logger.info("Survivor-free backtest completed",
            ...
            if 'error' in results:
                ...
                if look_ahead.get('micro_trading_detected'):
                    print("WARNING: LOOK-AHEAD BIAS: Micro-trading detected (very short trade durations)")
                else:
                ...
                if survivorship.get('potential_bias'):
                    ...
                    print(f"WARNING: SURVIVORSHIP BIAS: Low selection rate ({selection_rate:.1%})")
                else:
                ...
                if overfitting.get('unrealistic_sharpe'):
                    ...
                    print(f"WARNING: OVERFITTING: Unrealistically high Sharpe ratio ({sharpe:.2f})")
                else:
