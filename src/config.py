...
class Config:
    ...
    def validate(self) -> bool:
        ...
        """
        required_sections = [
            'system', 'universe', 'signals', 'risk_management',
            'macro_risk', 'data_sources', 'trading'
        ]
        ...
        try:
            ...
            if not isinstance(max_position_pct, (int, float)) or not 0 < max_position_pct <= 1:
                print(f"Error: max_position_percent must be between 0 and 1, got {max_position_pct}")
                return False
            ...
            if not isinstance(cri_threshold, (int, float)) or not 0 <= cri_threshold <= 1:
                print(f"Error: cri_threshold must be between 0 and 1, got {cri_threshold}")
                return False
            ...
            if not isinstance(g_score_threshold, (int, float)) or g_score_threshold <= 0:
                print(f"Error: g_score_threshold must be positive, got {g_score_threshold}")
                return False

            # Validate trading schedule
            run_time = self.get('trading.schedule.run_time', '16:00')
            if not isinstance(run_time, str) or len(run_time) != 5 or run_time[2] != ':':
