...
from data_processing import calculate_returns as dp_calculate_returns
from data_processing import calculate_volatility as dp_calculate_volatility
from data_processing import calculate_z_score as dp_calculate_z_score
...
def align_data_by_date(df1, df2, date_col: str = None):
    ...
    try:
        ...
        logging.exception(f"Failed to align data by date: {e}")
        return pd.DataFrame(), pd.DataFrame()
...
def calculate_performance_metrics(equity_curve) -> dict[str, float]:
    ...
    try:
        ...
        logging.exception(f"Failed to calculate performance metrics: {e}")
        return {}
...
def validate_data_quality(data, required_columns: list[str] = None) -> bool:
    ...
    try:
        ...
        logging.exception(f"Data validation failed: {e}")
        return False
...
def load_json_safe(file_path: str, default: dict = None) -> dict:
    ...
    try:
        ...
        logging.exception(f"Failed to load JSON from {file_path}: {e}")
        return default or {}
...
def save_json_safe(data: dict, file_path: str) -> bool:
    ...
    try:
        ...
        logging.exception(f"Failed to save JSON to {file_path}: {e}")
        return False
...
def is_market_open(exchange: str = "NYSE") -> bool:
    ...
    try:
        ...
        logging.exception(f"Failed to check market hours: {e}")
        return False


def calculate_position_size_fixed_fractional(
    equity: float, risk_percent: float, entry_price: float, stop_price: float
) -> int:
    """
    ...
    try:
        ...
        logging.exception(f"Failed to calculate position size: {e}")
        return 0


def calculate_position_size_percentage(
    equity: float, position_percent: float, price: float
) -> int:
    """
