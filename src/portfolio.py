...
from models import Order, Position, TradeHistory, get_portfolio_db_manager
...
class PortfolioManager:
    ...
    def _load_positions(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.positions)} open positions from database"
            )
    ...
    def execute_entry_order(self, signal: dict[str, Any], current_price: float) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Executed {side} order for {symbol}: "
            ...
            self.logger.error(
                f"Failed to execute entry order for {signal['symbol']}: {e}"
            )
            return False
    ...
    def execute_exit_order(
        ...
        try:
            ...
            trade_record = self.risk_manager.remove_position(
                symbol, exit_price, exit_reason
            )

            if not trade_record:
                self.logger.error(
                    f"Failed to remove position from risk manager: {symbol}"
                )
                return False
            ...
            self.logger.info(
                f"Executed exit order for {symbol}: "
    ...
    def get_portfolio_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.trade_history:
                durations = [
                    trade["duration"].total_seconds() / 86400
    ...
    def _save_position(self, position: dict[str, Any]) -> None:
        ...
        try:
            ...
            existing = (
                session.query(Position).filter_by(symbol=position["symbol"]).first()
            )
