...
try:
    from prometheus_client import Gauge, start_http_server, CollectorRegistry, generate_latest, Histogram, Counter
...
class HarvesterMetrics:
    ...
    def __init__(self, registry: Optional[CollectorRegistry] = None):
        ...
        if not PROMETHEUS_AVAILABLE:
            harvester_logger.warning("Prometheus client not available - metrics disabled")
            self.enabled = False
            return
    ...
    def update_signal_metrics(self, conviction: float = 0.5,
                            signal_type: str = "unknown") -> None:
        ...
        self.signals_generated.labels(signal_type=signal_type, outcome="generated").inc()
    ...
    def update_bias_metrics(self, bias_analysis: Dict[str, Any]) -> None:
        ...
        self.survivorship_bias_gauge.set(1 if survivorship.get('detected', False) else 0)
    ...
    def update_system_health(self, health_status: Dict[str, Any]) -> None:
        ...
        available_count = sum(1 for source, available in data_sources.items() if available)
    ...
    def start_server(self, port: int = 8000) -> None:
        ...
        if not self.enabled or not start_http_server:
            harvester_logger.warning("Cannot start metrics server - Prometheus not available")
            return
...
_metrics: Optional[HarvesterMetrics] = None
