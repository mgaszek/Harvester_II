...
class TestSignalCalculator:
    ...
    def test_calculate_cri_perfect_correlation(self, sample_signal_calculator, sample_price_data, sample_trends_data):
        ...
        price_values = []
        trends_values = []
        for i in range(12):  # Create 12 data points to ensure we have enough after pct_change
        ...
        with patch.object(sample_signal_calculator, '_align_data_by_date') as mock_align:
            aligned_data = pd.DataFrame({
                'price': price_values,
                'trends': trends_values
            })
            mock_align.return_value = aligned_data

            cri = sample_signal_calculator.calculate_cri('SPY', sample_price_data, sample_trends_data)
    ...
    def test_calculate_panic_score_normal_conditions(self, sample_signal_calculator, sample_price_data, sample_trends_data):
        """Test panic score calculation under normal market conditions."""
        panic_score = sample_signal_calculator.calculate_panic_score('SPY', sample_price_data, sample_trends_data)
    ...
    def test_calculate_panic_score_high_volatility(self, sample_signal_calculator):
        ...
        panic_score = sample_signal_calculator.calculate_panic_score('SPY', high_vol_price, trends_data)
    ...
    def test_calculate_panic_score_empty_data(self, sample_signal_calculator):
        ...
        panic_score = sample_signal_calculator.calculate_panic_score('SPY', empty_data, empty_data)
        assert panic_score == 0.0

    def test_calculate_g_score(self, sample_signal_calculator, mock_data_manager):
        """Test G-Score calculation."""
        # Mock macro data
        mock_data_manager.get_macro_indicator.side_effect = lambda indicator, **kwargs: {
            'VIX': pd.DataFrame({'value': [15.0, 16.0, 14.0]}, index=pd.date_range('2023-01-01', periods=3)),
            'SPY': pd.DataFrame({'value': [400.0, 405.0, 410.0]}, index=pd.date_range('2023-01-01', periods=3)),
            'USO': pd.DataFrame({'value': [70.0, 68.0, 72.0]}, index=pd.date_range('2023-01-01', periods=3))
