...
class RiskManager:
    ...
    def __init__(self, config):
        ...
        self.base_position_fraction = self.config.get('risk_management.base_position_fraction', 0.005)
        self.max_open_positions = self.config.get('risk_management.max_open_positions', 4)
        self.daily_drawdown_limit = self.config.get('risk_management.daily_drawdown_limit', 0.02)
        
        # Position sizing limits
        self.min_position_size = self.config.get('risk_management.position_sizing.min_position_size', 100)
        self.max_position_size = self.config.get('risk_management.position_sizing.max_position_size', 5000)
        self.risk_per_trade = self.config.get('risk_management.position_sizing.risk_per_trade', 0.005)
        
        # ATR-based stop loss and profit targets
        self.atr_profit_multiplier = self.config.get('risk_management.atr_profit_target_multiplier', 2.0)
        self.atr_stop_multiplier = self.config.get('risk_management.atr_stop_loss_multiplier', 1.0)
        ...
        self.high_risk_multiplier = self.config.get('macro_risk.position_size_multiplier.high_risk', 0.5)
        self.normal_risk_multiplier = self.config.get('macro_risk.position_size_multiplier.normal_risk', 1.0)
    ...
    def calculate_position_size(self, symbol: str, entry_price: float, 
                              atr: float, g_score: float = 0.0) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if g_score >= self.g_score_threshold:
                ...
                self.logger.info(f"High macro risk detected (G-Score: {g_score}), reducing position size")
            else:
            ...
            self.logger.info(f"Position sizing for {symbol}: {shares} shares, "
    ...
    def check_drawdown_limit(self) -> bool:
        ...
        try:
            ...
            if current_drawdown_amount >= daily_drawdown_amount:
                self.logger.critical(f"DAILY DRAWDOWN LIMIT EXCEEDED: "
                                  f"${current_drawdown_amount:,.2f} >= ${daily_drawdown_amount:,.2f}")
                return True
            
            self.logger.debug(f"Drawdown check: {self.current_drawdown:.2%} "
    ...
    def can_open_new_position(self) -> bool:
        ...
        try:
            ...
            if len(self.open_positions) >= self.max_open_positions:
                self.logger.info(f"Maximum open positions reached: {len(self.open_positions)}")
                return False
            ...
            if available_equity < self.min_position_size:
                self.logger.info(f"Insufficient equity for new position: ${available_equity:,.2f}")
                return False
    ...
    def add_position(self, position_info: Dict[str, float]) -> bool:
        ...
        try:
            ...
            if not self.can_open_new_position():
                self.logger.warning(f"Cannot add position for {symbol} - limits exceeded")
                return False
            
            # Add position with trailing stop initialization
            position_info['trailing_stop'] = position_info['stop_loss_price']  # Initialize trailing stop
            ...
            self.logger.info(f"Added position for {symbol}: "
    ...
    def remove_position(self, symbol: str, exit_price: float, 
                       exit_reason: str = "manual") -> Dict[str, float]:
        """
    ...
    def check_exit_signals(self, current_prices: Dict[str, float]) -> List[Dict[str, float]]:
        """
    ...
    def get_portfolio_summary(self) -> Dict[str, any]:
        ...
        try:
            total_position_value = sum(pos['position_value'] for pos in self.open_positions.values())
            total_risk_amount = sum(pos['risk_amount'] for pos in self.open_positions.values())
        ...
        try:
            ...
            for symbol, position in self.open_positions.items():
                ...
                if position['side'] == 'BUY':
                    ...
                    potential_stop = min(potential_stop, current_price - min_stop_distance)

                    # Only update if new stop is higher than current stop
                    if potential_stop > position.get('trailing_stop', position['stop_loss_price']):
                        position['trailing_stop'] = potential_stop
                        position['stop_loss_price'] = potential_stop  # Update the actual stop
                        updated_stops += 1
                        self.logger.debug(f"Updated trailing stop for {symbol}: ${potential_stop:.2f}")
                    ...
                    # Ensure minimum distance from current price
                    min_stop_distance = current_price * self.trailing_min_distance
                    potential_stop = max(potential_stop, current_price + min_stop_distance)
            ...
            if updated_stops > 0:
                self.logger.info(f"Updated trailing stops for {updated_stops} positions")

        except Exception as e:
            self.logger.error(f"Failed to update trailing stops: {e}")

    def get_trailing_stop_info(self, symbol: str) -> Optional[Dict[str, float]]:
        """
